...
import {ApiCreatedResponse, ApiImplicitQuery, ApiUseTags} from '@nestjs/swagger';
...
export class HistoryController {
    ...
    async addTodoToBike(@Query('bikeId') bikeId: string,
                        @Query('transferred', new ToBooleanPipe()) isTransferred: boolean = false,
                        @Body() vm: TodoVm): Promise<TodoVm> {
        const todo = await this.historyService.addTodoToBike(bikeId, isTransferred, vm);
    ...
    async addStoryToBike(@Query('bikeId') bikeId: string,
                         @Body() vm: StoryVm): Promise<StoryVm> {
    ...
    async addTodoToPc(@Query('pcId') pcId: string,
                      @Query('transferred', new ToBooleanPipe()) isTransferred: boolean = false,
                      @Body() vm: TodoVm): Promise<TodoVm> {
    ...
    async addStoryToPc(@Query('pcId') pcId: string,
                       @Body() vm: StoryVm): Promise<StoryVm> {
