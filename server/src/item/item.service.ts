...
export class ItemService extends BaseService<Item> {
  ...
  constructor(
    @InjectModel('Item') private readonly _itemModel: Model<Item>,
  ...
  async createBaseItem(
    ...
    const newItem = this.createModel({
      donorId: donor.id,
      type: itemVm.type,
      notes: itemVm.notes,
    });
  ...
  async updateBike(id: string, vm: BikeVm): Promise<Bike> {
    ...
    return this._bikeModel
      .findByIdAndUpdate(bike.id, bike, { new: true })
      .populate('images')
      .populate('todos')
      .populate('stories');
  }
  ...
  async updatePc(id: string, vm: PCVm): Promise<PC> {
    ...
    return this._pcModel
      .findByIdAndUpdate(pc.id, pc, { new: true })
      .populate('images')
      .populate('todos')
      .populate('stories');
  }
