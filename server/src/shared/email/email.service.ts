...
export class EmailService {
    ...
    constructor() {
        this._emailAuthUsername = process.env.EMAIL_USERNAME || 'nartc1410@gmail.com';
    ...
    async sendEmail(templateName: EmailTemplates, to: string | string[], from?: string, data?: EmailContent) {
        if (!this.isTemplateExist(templateName)) {
            throw new InternalServerErrorException(`Template not found ${templateName}`);
        }
    ...
    private async prepareTemplate(templateName: string, data: EmailContent): Promise<EmailData> {
        try {
            const emailContent: EmailData = await this._email.renderAll(templateName, data);
